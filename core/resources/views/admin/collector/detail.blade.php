...
@section('panel')
                        ...
                        <div class="row g-0">
                            <div class="col-sm-6">
                                <div class="widget-card widget--primary">
                                    <a href="{{ route('admin.report.collector.transaction') }}?driver_id={{ $driver->id }}"
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    <a href="{{ route('admin.report.user.payment') }}?driver_id={{ $driver->id }}"
                                        class="widget-card-link"></a>
                                    <div class="widget-card-left">
                                        <div class="widget-icon">
    ...
    <div class="row responsive-row">
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.pickups.running') }}?driver_id={{ $driver->id }}"
                :currency="false" variant="primary" title="Running Waste Pickup" :value="$widget['running_ride']" icon="las la-spinner" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.pickups.completed') }}?driver_id={{ $driver->id }}"
                :currency="false" variant="success" title="Completed Waste Pickups" :value="$widget['completed_ride']" icon="las la-check-double" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.pickups.canceled') }}?driver_id={{ $driver->id }}"
                :currency="false" variant="danger" title="Canceled Waste Pickup" :value="$widget['canceled_ride']" icon="las la-times-circle" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.pickups.all') }}?driver_id={{ $driver->id }}" :currency="false"
                variant="info" title="Total Waste Pickups" :value="$widget['total_ride']" icon="las la-list" />
        </div>
    </div>

    <div class="row responsive-row">
        <div class="col-xxl-8">
            <form action="{{ route('admin.collector.update', $driver->id) }}" method="POST" enctype="multipart/form-data"
                class="user-form">
                @csrf
                <div class="card">
                    ...
                    @if ($driver->driver_data && $driver->dv == Status::PENDING)
                        <div class=" d-flex gap-3 flex-lg-wrap mt-3">
                            <button class="btn btn--success confirmationBtn flex-fill" data-question="@lang('Are you sure to approve this document?')"
                                data-action="{{ route('admin.collector.verification.approve', $driver->id) }}">
                            ...
                            <button class="btn btn--danger confirmationBtn flex-fill" data-question="@lang('Are you sure to reject this driver document?')"
                                data-action="{{ route('admin.collector.verification.reject', $driver->id) }}">
                    ...
                    @if (!blank(@$driver->vehicle_data) && $driver->vv == Status::PENDING)
                        <div class=" d-flex gap-2 flex-lg-wrap mt-3">
                            <button class="btn btn--success confirmationBtn" data-question="@lang('Are you sure to approve this document?')"
                                data-action="{{ route('admin.collector.vehicle.approve', $driver->id) }}">
                                <i class="las la-check me-1"></i>@lang('Approve')
                            </button>
                            <button class="btn btn--danger confirmationBtn" data-question="@lang('Are you sure to reject this driver document?')"
                                data-action="{{ route('admin.collector.vehicle.reject', $driver->id) }}">
                                <i class="las la-ban me-1"></i>@lang('Reject')
                            </button>
                        </div>
                    @endif
    ...
    <x-admin.ui.modal id="driverStatusModal">
            ...
            <div>
                <h4 class="modal-title">
                    @if ($driver->status == Status::USER_ACTIVE)
                        @lang('Ban Waste Collector')
                    @else
                        @lang('Unban Waste Collector')
                    @endif
                </h4>
                @if ($driver->status == Status::USER_ACTIVE)
                    <small>@lang('If this waste collector is banned, they will no longer have access to their dashboard.')</small>
                @else
                    <small>
        ...
        <x-admin.ui.modal.body>
            <form action="{{ route('admin.collector.status', $driver->id) }}" method="POST">
                @csrf
                @if ($driver->status == Status::USER_ACTIVE)
                    <div class="form-group">
    ...
    <x-admin.ui.modal id="addSubModal">
        <x-admin.ui.modal.header>
            <div>
                ...
                <small class="modal-subtitle">@lang('Add funds to waste collector accounts by entering the desired amount  below')</small>
            </div>
            <button type="button" class="btn-close close" data-bs-dismiss="modal" aria-label="Close">
                <i class="las la-times"></i>
            </button>
        ...
        <x-admin.ui.modal.body>
            <form method="POST" action="{{ route('admin.collector.add.sub.balance', $driver->id) }}">
                @csrf
    ...
    <div class=" d-flex gap-2  flex-wrap">
        ...
        @if ($driver->status == Status::USER_ACTIVE)
            <button type="button" class="flex-fill btn  btn--warning" data-bs-toggle="modal"
                ...
                <i class="las la-ban me-1"></i>@lang('Ban Waste Collector')
            </button>
        @else
            <button type="button" class="flex-fill btn  btn--info" data-bs-toggle="modal"
                data-bs-target="#driverStatusModal">
                <i class="las la-ban me-1"></i>@lang('Unban Waste Collector')
            </button>
        @endif
        <a href="{{ route('admin.report.collector.notification.history') }}?driver_id={{ $driver->id }}"
