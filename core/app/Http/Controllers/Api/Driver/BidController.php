...
class BidController extends Controller
{
    public function create(Request $request, $id)
    {
        ...
        $ride   = WastePickup::pending()->find($id);
        ...
        $driver = Collector::where('online_status', Status::YES)
            ->where("waste_service_id", $ride->service_id)
        ...
        $bidExists = Bid::where('ride_id', $id)->where('collector_id', $driver->id)->whereIn('status', [Status::BID_PENDING, Status::BID_ACCEPTED])->first();
    ...
    public function cancel($id)
    {
        ...
        $driver = auth()->user();
        $bid    = Bid::where('collector_id', $driver->id)->where('status', Status::BID_PENDING)->where('ride_id', $ride->id)->first();
        if (!$bid) {
