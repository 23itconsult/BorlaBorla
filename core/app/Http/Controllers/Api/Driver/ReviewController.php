...
class ReviewController extends Controller
{
    public function review()
    {
        ...
        return apiResponse("review", "success", $notify, [
            ...
            "reviews"           => Review::latest('id')->where('collector_id', $driver->id)->with('ride.user')->get(),
            'driver'            => $driver
        ]);
    }
    ...
    public function reviewStore(Request $request, $id)
    {
        ...
        $driver = auth()->user();
        $ride   = Ride::completed()->where('collector_id', $driver->id)->find($id);
