...
use App\Events\WastePickup as EventsPickup;
...
class PaymentController extends Controller
{
    ...
    {
        $pageTitle = 'Deposit History';
        $deposits = auth()->guard('collector')->user()->deposits()->searchable(['trx'])->with(['gateway'])->orderBy('id', 'desc')->paginate(getPaginate());
    ...
    public function appDepositConfirm($hash)
    {
        ...
        if ($data->user_id) {
            ...
            $driver = Collector::findOrFail($data->collector_id);
            auth()->guard('collector')->login($driver);
    ...
    public function depositConfirm()
    {
        ...
        $view = $deposit->collector_id ? str_replace('user', 'driver', $data->view) : $data->view;
    ...
    public static function userDataUpdate($deposit, $isManual = null)
    {
        ...
        if ($deposit->status == Status::PAYMENT_INITIATE || $deposit->status == Status::PAYMENT_PENDING) {
            ...
            if ($deposit->pickup_id) {
                ...
                $ride = WastePickup::findOrFail($deposit->pickup_id);
                (new RidePaymentManager())->payment($ride, Status::PAYMENT_TYPE_GATEWAY);
                $ride->load('collector', 'user');
                initializePusher();

                event(new EventsPickup($ride, 'online-payment-received'));
            } else {

                $driver = Collector::find($deposit->collector_id);
