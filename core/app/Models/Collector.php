...
class Collector extends Authenticatable
{
    ...
    {
        return $this->hasMany(DeviceToken::class, 'collector_id');
    }

    public function bid()
    {
        return $this->hasMany(Bid::class, 'collector_id');
    }
    ...
    {
        return $this->hasMany(Deposit::class, 'collector_id')->where('status', '!=', Status::PAYMENT_INITIATE);
    }
    ...
    {
        return $query->whereDoesntHave('pickup', function ($pickup) {
            $pickup->running();
        });
    }
    ...
    {
        return new Attribute(
            get: fn() => getImage(getFilePath('collector') . '/' . $this->image, getFilePath('collector'), isAvatar: true),
        );
    }
